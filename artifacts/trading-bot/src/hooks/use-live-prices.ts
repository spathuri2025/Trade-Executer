 ...
 * (`/api/stream/prices`, per-user underneath — see
 * .agents/memory/multi-tenant-broker.md — but this hook needs no awareness of
 * that; the session cookie routes it server-side). A single EventSource is
 * opened for the whole app and ref-counted by subscribers, so mounting many
 * price consumers never opens more than one upstream connection.
 *
