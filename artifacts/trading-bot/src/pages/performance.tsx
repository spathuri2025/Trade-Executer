  ...
  return (
    <div className="space-y-6 md:space-y-8">
      ...
      {data && (
        <p className="text-xs" style={{ color: muted }}>
          {data.results.length > 0
            ? `${data.broker} · MA ${data.shortPeriod}/${data.longPeriod} · up to ${data.historyBars} bars · generated ${new Date(data.generatedAt).toLocaleString()}`
            : ""}
        </p>
      )}
      ...
      <p className="text-[11px] leading-relaxed pt-2" style={{ color: muted }}>
        Backtests use recent hourly closes and a simplified always-in-market model (no spread,
        slippage, or overnight financing). Past performance does not guarantee future results and
