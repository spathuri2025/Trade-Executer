...
async function createSession(): Promise<Session> {
  const apiKey =
    process.env.CAPITAL_COM_API_KEY ?? process.env.CAPITALCOM_API_KEY;
  const password =
    process.env.CAPITAL_COM_PASSWORD ?? process.env.CAPITALCOM_PASSWORD;
  const identifier =
    process.env.CAPITAL_COM_EMAIL ??
    process.env.CAPITAL_COM_IDENTIFIER ??
    process.env.CAPITALCOM_IDENTIFIER;

  if (!apiKey || !password || !identifier) {
    throw new Error(
      "Capital.com credentials missing: set CAPITAL_COM_API_KEY, CAPITAL_COM_PASSWORD and CAPITAL_COM_EMAIL",
    );
  }
